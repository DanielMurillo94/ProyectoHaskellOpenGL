...
data Balas = NullB | Bala {pX::Float, pY::Float, pZ::Float, vel::Float, vivo::Int, sig::Balas} deriving(Show, Eq)
...
main = do
    ...
    idleCallback $= Just (temporizador tiempo posicionx posiciony vposx vposy dispara)
...
temporizador tiempo angulox anguloy vangulox vanguloy dispara= do
    ax <- get angulox
    ay <- get anguloy
    vx <- get vangulox
    vy <- get vanguloy
...
--eliminaBalas :: Balas -> Balas
--eliminaBalas NullB = NullB
--eliminaBalas 

destruyeBalas :: Balas -> Balas
destruyeBalas NullB = NullB
destruyeBalas (Bala x y z v _ sig ) = destruyeBala x y z v (Bala x y z v 1 sig)

destruyeBala :: Float -> Float -> Float -> Float -> Balas -> Balas
destruyeBala x y z v NullB = NullB
destruyeBala x y z v (Bala i j k l _ sig) = if (x == i && y == j && z > 5) then sig else destruyeBala x y z v sig
