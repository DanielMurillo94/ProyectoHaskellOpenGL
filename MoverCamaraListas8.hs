...
main = do
    ...
    puntajeM <- newIORef 1.0
    puntaje <- newIORef 1.0
    displayCallback $= desplegar posicionx posiciony dispara enemigos tiempo randind puntajeM
    idleCallback $= Just (temporizador tiempo posicionx posiciony vposx vposy dispara enemigos frecenem randind dfrecenem puntaje)
    keyboardMouseCallback $= Just (teclado vposx vposy dispara posicionx posiciony)
    mainLoop

desplegar posicionx posiciony dispara enemigos tiempo puntaje puntajeM= do
    px <- get posicionx
    py <- get posiciony
    en <- get enemigos
    b <- get dispara
    ...
    renderString TimesRoman24 $ "Tiempo: " ++ show(p)
...
temporizador tiempo angulox anguloy vangulox vanguloy dispara enemigos frecenem randind dfrecenem puntaje= do
    ax <- get angulox
    ay <- get anguloy
    vx <- get vangulox
    vy <- get vanguloy
...
muere :: Cubos -> Integer
muere NullC = 0
muere (Cubo x y z v sig) = if z > 0 then muere sig else 1
